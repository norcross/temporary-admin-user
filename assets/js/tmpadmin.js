...
function tempAdminRemoveRows( remRows, boxBlock ) {
	...
	// start looping through my IDs and remove the rows
	for ( var i = 0; i < remRows.length; i++ ) {
		jQuery( 'div#' + boxBlock + ' div.tempadmin-users-list-data' ).find( 'tr#single-user-' + remRows[i] ).remove();
	}

	// check if we have any rows left or not and handle accordingly
	if ( jQuery( 'div#' + boxBlock + ' div.tempadmin-users-list-data tbody' ).has( 'tr.tempadmin-single-user-row' ).length ) {
		// we have rows left, so just reset the classes
		tempAdminResetRowClass( boxBlock );
	} else {
		// if we removed them all, replace the rows with our spacer
		jQuery( 'div#' + boxBlock + ' div.tempadmin-users-list-data' ).find( 'tbody' ).append( tempAdminData.emptyRow );
	}
